...
async function callLLMBrain(text, currentDate) {
    ...
    if (geminiKey) {
        try {
            ...
            if (res.ok) {
                ...
                if (raw)
                    return { data: JSON.parse(raw), model: 'Gemini Flash Lite' };
            }
            else {
    ...
    if (dashKey) {
        try {
            ...
            if (res.ok) {
                ...
                if (raw)
                    return { data: JSON.parse(raw), model: 'Qwen-Turbo' };
            }
        }
        catch (e) {
    ...
    if (groqKey) {
        try {
            ...
            if (res.ok) {
                ...
                const raw = d.choices?.[0]?.message?.content;
                if (raw)
                    return { data: JSON.parse(raw), model: 'Llama-3.3-70B' };
            }
        }
        catch (e) {
...
const processVoice = async (req, res, next) => {
    ...
    try {
        ...
        if (rawPersonName.toLowerCase() === 'asama')
            rawPersonName = 'Usama';
        ...
        const direction = parsedData?.transaction?.direction === 'got' ? 'got' : 'gave';
